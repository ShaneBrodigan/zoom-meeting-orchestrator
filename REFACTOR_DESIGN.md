...
5. Add VM5 iperf noise — **design converged (grill-me 2026-06-10, decision 10); BUILT + unit-verified +
   ...
   **dedicated internet iperf server**; labeler separates flows by **destination IP**. **Live AWS
   validation deferred to one combined run with the labeler (step 6).** Infra prereqs still open (Shane):
   internet iperf server (SG TCP+UDP open from VM4 EIP `34.242.98.206`), Docker+iperf3 on VM5,
   `config/noise.json` in S3 (not created yet).
6. Offline labeler.
